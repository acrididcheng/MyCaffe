			 ...
			 len, loff_t * off)
{
    ...
    memset(obj_dev->ds_axidma_addr, 0, sizeof(obj_dev->ds_axidma_addr));
	...
	printk(KERN_INFO "third11\n");
	dmaSynchS2MM(obj_dev);
	printk(KERN_INFO "third23\n");
	printk(KERN_INFO "read:%x\n",obj_dev->ds_axidma_addr);
	copy_to_user(buf, obj_dev->ds_axidma_addr, len);
			  ...
			  size_t len, loff_t * off)
{
	...
	printk(KERN_INFO "buf:%u\n", len);
	printk(KERN_INFO "write:%x\n",obj_dev->ds_axidma_addr);
    copy_from_user(obj_dev->ds_axidma_addr,buf, len);
...
static int ds_axidma_pdrv_probe(struct platform_device *pdev)
{
    ...
    memcpy(obj_dev->ds_axidma_addr, "7654321", 8);
